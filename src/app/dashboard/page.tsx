...
export default async function TrainingPage(props: PageProps) {
  ...
  // Selected week via query param ?week=YYYY-MM-DD (Monday), fallback to current week
                  ...
                  <div className="flex flex-col gap-2">
                    {items.map((it: SessionSummary) => {
                      const durationSec: number | null | undefined =
                        it.durationSec ?? null;
                      const title = `${it.templateName} — ${formatDuration(durationSec)}`;
